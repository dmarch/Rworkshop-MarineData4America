...
This tutorial uses two sample datasets, that will be introduced later on in this tutorial. First, we will create a new directory in current working directory to write data to

```{r message = FALSE, warning = FALSE}
dir.create("data")
```

Then, we will download the sample data
...
The numerical model can be considered as a gridded product. To manipulate and visualize gridded data, we will use the `raster` package.
...
Here, we will combine all monthly maps (n=12) to calculate the mean and standard deviation (SD). These constitutes two common metrics used to describe de centrality (middle value) and dispersion (how spread out the values are from the average) of the data. We will use the function `calc()`, which gathers all monthly for each single grid cell and applies the given function (i.e. mean or sd).
...
We can visualize the boundaries of Galapagos Islands MPA, with the basic `plot()`.
...
However, in this occasion, we lack context information. In order to improve the visualization, we will use the ***leaflet*** package, which provides dynamic visualizations and uses web services to add base maps. In this plot, we will combine the previous average map of SST with the boundary of the MPA.
...
Here we will summarize raster values over polygonal areas, commonly referred to as zonal statistics. We will use the `extract()` function from ***raster*** package
...
Note we use `raster::extract` this time when using a function.   



## Plot time series

You can find further information about the ***ggplot2*** package [here](https://ggplot2.tidyverse.org/)
